...
public class ServerBootstrap extends AbstractBootstrap<ServerBootstrap, ServerChannel> {
    ...
    void init(Channel channel) {
        setChannelOptions(channel, newOptionsArray(), logger);  // 获取用户代码中配置的channelOptions属性设置到对应的NioServerSocketChannelConfig(EchoServer中.channelOptions设置的 即.option方法)
        setAttributes(channel, newAttributesArray());   // 获取用户代码中配置的attribute属性设置到对应的Channel(EchoServer中.attributes设置的 即.attr方法)

        ChannelPipeline p = channel.pipeline();

        // 以下4个属性是为了初始化SocketChannel而准备的
        // ====================================
        final EventLoopGroup currentChildGroup = childGroup;
        final ChannelHandler currentChildHandler = childHandler;
        // 保存用户代码中配置的childOptions(EchoServer中.childOptions设置的)
