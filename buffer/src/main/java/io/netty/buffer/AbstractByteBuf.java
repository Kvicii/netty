...
public abstract class AbstractByteBuf extends ByteBuf {
    ...
    /**
     * mark读指针变量
     * 见 {@link ByteBuf#markReaderIndex()}
     */
    private int markedReaderIndex;
    /**
     * mark写指针变量
     * 见 {@link ByteBuf#markWriterIndex()}
     */
    private int markedWriterIndex;
    /**
     * ByteBuf最多能分配的容量
     * 见 {@link ByteBuf#maxCapacity()}
     */
    ...
    public int writeBytes(ScatteringByteChannel in, int length) throws IOException {
        ensureWritable(length);
        // 通过原生的Channel把数据读到JDK原生的Bytebuffer 再从ByteBuf读到Netty中的ByteBuf
        int writtenBytes = setBytes(writerIndex, in, length);   // -1 代表 EOF 正常关闭 IO Exception代表读数据时被关闭
        if (writtenBytes > 0) {
            writerIndex += writtenBytes;
        }
        return writtenBytes;
